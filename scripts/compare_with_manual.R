...
flags = list(
            ...
            pilot.names = c("o1-preview", "l3.1_8b", "l3.1_70b", "l3.3_70b")  # set the corresponding pilot names for each folder of files
        )
...
full.consensus.matches = consensus.matches %>%
    ...
    filter(count>7) %>%  # choose what constitutes your consensus event set (by number of methods matching that event)
    select(-count) %>%
...
# AULTC
upper_limit = log(60*24*365.26)
ecdf_auc2 = function(df, def=0, upper=upper_limit) {
    df %>%
    bind_rows(tibble(field=upper)) %>%  # add a row for the last row field2 calculation, then remove it below
    mutate(field=ifelse(is.na(field), upper, field)) %>%
    arrange(field) %>% 
    mutate(field = pmin(field, upper)) %>%
    mutate(
        field2 = lead(field, default=def),
        # field2 = c(def, field[1:(n()-1)]),
        nth = (1:n())/(n()-1),
        dx = field2 - field,
        box = dx*nth 
    ) %>%
...
match.tbl %>% select(common.files, pilot.files, pilot.names, man.join.match.join.pilot) %>%
    ...
    filter(keep) %>% #nest(data=-file.name)
    mutate(ae = abs(as.numeric(time.pilot)-as.numeric(time))) %>%
    mutate(ae = ifelse(is.na(ae), upper_limit,ae)) %>%
    # nest(data=-Version)
    group_by(Version) %>%
    summarise(mae = median(ae), q25 = quantile(ae, 0.25), q75=quantile(ae,0.75))
